...
export default {
  ...
  mounted() {
    this.planFieldList = [
      ...
      { prop: 'contract_name', width: '200', label: '合同编号' },
    ...
    this.fieldList = [
      ...
      { prop: 'plan_id_info', width: '200', label: '期数' },
      { prop: 'owner_user_name', width: '200', label: '负责人' },
  ...
  methods: {
    ...
    handleFile(type, item) {
      if (type == 'edit') {
        ...
        this.$confirm('您确定要删除吗?', '提示', {
          ...
          .then(() => {
            crmReceivablesPlanDeleteAPI({ id: item.row.plan_id })
              .then(res => {
    ...
    fieldFormatter(row, column) {
      ...
      if (column.property === 'check_status') {
        return this.getCRMStatusName(row.check_status)
      } else if (column.property == 'contract_money' || column.property == 'money') {
