  ...
  methods: {
      ...
      request(params)
        .then(res => {
          ...
          this.list = res.data.list.map(item => {
            ...
            item.createUserName = item.create_user_id_info.realname
            item.source = item.source || '附件上传'
            return item
          })
        })
        .catch(() => {
    ...
    handleFile(type, item) {
      if (type === 'preview') {
        ...
        downloadFileAPI({ name: item.row.name, save_name: item.row.save_name }).then(res => {
        ...
        this.editForm.save_name = item.row.save_name
    ...
    confirmEdit() {
      if (this.editForm.name) {
        crmFileUpdateAPI({
          // file: this.editForm.data.row.file,
          name: this.editForm.name,
          save_name: this.editForm.save_name
        })
          .then(res => {
