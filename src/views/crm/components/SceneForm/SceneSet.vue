  ...
  methods: {
    getSceneList() {
      crmSceneSetIndexAPI({
        ...
        .then(res => {
          this.checkedLeftData = res.data.list.map(item => {
            item.check = false
            // item.data = item.data.length ? {} : JSON.parse(item.data)
            return item
          })
          this.checkedRightData = res.data.hideList.map(item => {
            item.check = false
            // item.data = item.data.length ? {} : JSON.parse(item.data)
            return item
          })
    ...
    handleConfirm() {
      ...
      var leftTemps = this.checkedLeftData.filter(item => {
        return item.scene_id == this.default_id
      })
    ...
    handleleftCheckAllChange(value) {
      ...
      this.checkedLeftData.forEach(item => {
        item.check = item.type == 1 ? false : value
      })
