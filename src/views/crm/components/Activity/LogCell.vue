  ...
  <div class="fl-c">
    <flexbox class="fl-h">
      <xr-avatar
        ...
        class="fl-h-img" />
      <div class="fl-h-b">
        <div class="fl-h-name">{{ item.create_user_name }}</div>
        <div class="fl-h-time">{{ item.update_time }}</div>
      </div>
      <div
        v-if="item.category"
        class="follow">
      ...
      <span
        ...
        <i
          ...
          :class="activityIcon" />
        <span class="log-mark__label">{{ getActivityTypeName(item.activity_type) + '-' + getRecordLogTypeName(item.type) }}</span>
      </span>
      <el-dropdown
    ...
    <div class="fl-b">
      ...
      <flexbox
        ...
        wrap="wrap">
        <div
          v-src:background-image="file.file_path"
          v-for="(file, index) in item.imgList"
          :key="file.file_path"
          class="fl-b-img-item"
      ...
      <div v-if="item.fileList && item.fileList.length > 0">
        <flexbox
          ...
          <div class="cell-ft">
            ...
            <a @click="previewImg(item.fileList, index)">预览</a>
            <a @click="downloadFile(file)">下载</a>
          </div>
        </flexbox>
      </div>

      <div v-if="item.contacts_list && item.contacts_list.length > 0">
        <flexbox
          align="stretch"
          class="cell">
          <div class="cell-hd">
          ...
          <div
            ...
            <a @click="checkRelationDetail('contacts', item.contacts_id)">{{ item.name }}</a>
          </div>
        </flexbox>
      </div>

      <div v-if="item.business_list && item.business_list.length > 0">
        <flexbox
          align="stretch"
          class="cell">
          <div class="cell-hd">
          ...
          <div class="cell-ft">
            <div
              ...
              :key="index"
              class="cell-ft__item text-one-line">
              <a @click="checkRelationDetail('business', item.business_id)">{{ item.name }}</a>
            </div>
          </div>
        </flexbox>
      </div>

      <div v-if="item.next_time" class="cell-top">
        <flexbox
          align="stretch"
          class="cell">
          <div class="cell-hd">
...
export default {
  ...
  computed: {
    ...mapGetters(['crm']),
    hasEditAuth() {
      return this.crm && this.crm.activity && this.crm.activity.update
    },

    hasDeleteAuth() {
      return this.crm && this.crm.activity && this.crm.activity.delete
    },


    activityIcon() {
      return this.getXrIcon(this.getActivityType(this.item.activity_type))
    },

    activityIconColor() {
      return this.getXrIconColor(this.getActivityType(this.item.activity_type))
    }
  },
  mounted() {},
  methods: {
    ...
    downloadFile(file) {
      downloadFile({ path: file.file_path, save_name: file.save_name, name: file.name })
    },
    /**
     * 删除跟进记录
     */
