      ...
      <el-form
        ...
        >
          <template slot-scope="{ data }">
            ...
            <xh-prouct-cate
              v-else-if="data && data.form_type == 'category'"
...
export default {
  ...
  watch: {
    'action.editDetail': function(data) {
      if (data) {
        this.imageData = {
          mainFile: data.cover_images || [],
          detailFile: data.details_images || []
        }
      }
    }
  },
    ...
    saveClick() {
      ...
      crmForm.validate(valid => {
        if (valid) {
          ...
          // 图片信息
          params.cover_images = this.imageData.mainFile ? this.imageData.mainFile.map(item => item.file_id).join(',') : ''
          params.details_images = this.imageData.detailFile ? this.imageData.detailFile.map(item => item.file_id).join(',') : ''
