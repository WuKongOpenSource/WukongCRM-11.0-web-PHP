    ...
    <div
      ...
      <el-form ref="form" :model="form" label-width="100px">
        <template v-for="(item, index) in fieldList">
          <el-form-item v-if="item.field == 'title_type'" :key="index" :label="item.name" :prop="item.field">
            <el-select
...
export default {
  ...
  computed: {
    ...
    contentHeight() {
      return this.form.title_type == 2 ? '190px' : '300px'
    }
  },
  watch: {
    visible() {
  ...
  methods: {
    getFieldList() {
      const hidden = this.detail && (this.detail.titleType || this.form.title_type) == 2
      this.fieldList = [{
        name: '抬头类型',
        field: 'title_type',
        formType: 'select',
    ...
    valueChange() {
      const hidden = (this.form.title_type || this.form.title_type) == 2
    ...
    handleConfirm() {
      ...
      if (this.form.title_type == 2) {
        params.title_type = this.form.title_type
        params.invoice_title = this.form.invoice_title
      ...
      if (this.detail) {
        params.info_id = this.detail.info_id
        delete params.create_user_name
