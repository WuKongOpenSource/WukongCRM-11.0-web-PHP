...
export default {
  ...
  data() {
    return {
      ...
      fieldReportList: [
        { prop: 'business_name', label: '商机名称' },
        { prop: 'customer_name', label: '客户名称' },
        { prop: 'business_type', label: '商机状态组' },
        { prop: 'business_stage', label: '商机阶段' },
        { prop: 'money', label: '商机金额' },
        { prop: 'deal_date', label: '预计成交日期' },
        { prop: 'owner_user_name', label: '负责人' },
      ...
      biBusinessTrendAPI(this.postParams)
        .then(res => {
          ...
          for (let index = 0; index < this.axisList.length; index++) {
            ...
            listData[`type${index}`] = element.business_num
          }
