...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.request({ page: this.page, limit: 15, ...this.params })
        .then(res => {
          ...
          if (!this.noMore) {
            this.list = this.list.concat(res.data.list.page.list)
            this.page++
          }

          this.totalCount = res.data.list.page.dataCount

          this.loading = false
          this.noMore = !(res.data.list.page.list && res.data.list.page.list.length == 15)
        })
        .catch(() => {
          this.noMore = true
          this.loading = false
    ...
    examineCellHandle(data) {
      // 编辑
      if (data.type == 'view') {
        this.relatedID = data.data.item.examine_id
