    ...
    <div class="examine-cell">
      <flexbox class="person">
        ...
        <div class="person__bd">
          <div class="person__bd-name">{{ data.create_user_info.realname }}</div>
          <div class="person__bd-des">{{ `创建审批于 ${data.create_time}` }}</div>
        </div>
      ...
      <div class="examine-cell__bd">
        ...
        <flexbox class="examine-content">
          ...
          <div class="examine-content__bd">
            <div>
              <a @click="handleClick('detail')">{{ data.category_name }}</a>
            </div>
            <div
        ...
        <div class="examine-stage">
          <i :class="getStatusIcon(data.check_status)" :style="{ color: getTaskStatusColor(data.check_status) }" />
          <span>{{ `${data.examine_name || ''}${getTaskStatusName(data.check_status)}` }}</span>
        </div>
            ...
            <flexbox
              ...
              <div class="relate-cell__bd">
                <div
                  ...
                  <a @click="checkRelationDetail(item.type, child[`${item.type}_id`] || child.id)">{{ child[`${item.type}_name`] || child.name }}</a>
                </div>
              </div>
            </flexbox>
          </div>
  ...
  computed: {
    ...
    moreTypes() {
      ...
      if (this.data.permission) {
        if (this.data.permission.is_recheck) {
          mores.push({ type: 'withdraw', name: '撤销', icon: 'reset' })
        }
        if (this.data.permission.is_update) {
          mores.push({ type: 'edit', name: '编辑', icon: 'edit' })
        }

        if (this.data.permission.is_delete) {
     ...
     */
    canCheck() {
      return this.data.permission && this.data.permission.is_check
    }
  },
  watch: {},
  mounted() {},
