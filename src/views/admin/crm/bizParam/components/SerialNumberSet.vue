    ...
    <div class="content-body">
      ...
      <div v-for="(listItem, index) in list" :key="index" class="rule-section">
        <div class="rule-header">
          <span class="rule-header-title">{{ listItem.number_type | labelName }}</span>
          <el-switch
              ...
              <div v-for="(typeItem, typeIndex) in listItem.setting" :key="'rule'+typeIndex" class="rule">
                ...
                <template>
                  ...
                  <template v-else-if="typeItem.type == 3">
                    ...
                    <el-input v-model="typeItem.increase_number" :maxlength="9" class="rule-code-input" type="number" @input="if(typeItem.increase_number.length>9)typeItem.increase_number=typeItem.increase_number.slice(0,9)" />
                    <span class="rule-code-span">重新开始编号</span>
                    <el-select v-model="typeItem.reset" class="rule-code-input" >
                      <el-option
        ...
        for (let index = 0; index < setting.length; index++) {
          ...
          if (element.value) {
            if (element.type == 1) {
              ...
              const interalValue = parseInt(element.increase_number || 0)
              if (i == 1) {
    ...
    save() {
      ...
      for (let bIndex = 0; bIndex < this.list.length; bIndex++) {
        ...
        for (let index = 0; index < big.setting.length; index++) {
          ...
          if (item.type == 3) {
            if (item.increase_number === '' ||
          item.increase_number === undefined ||
           item.value === '' ||
