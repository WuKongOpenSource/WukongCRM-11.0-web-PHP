...
export default {
  ...
  watch: {
    infoData: function(val) {
      this.name = val.name
      const businessDep = val.businessDep
      this.businessDep = businessDep.filter(item => item > 0).map(Number)
      ...
      if (!this.name) {
        ...
        if (pass) {
          ...
          const params = {
            ...
            // },
            name: this.name,
            structure_id: this.businessDep,
            status: this.settingList,
            type_id: this.infoData.type_id
          }
