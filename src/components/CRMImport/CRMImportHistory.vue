  ...
  <div class="import-history">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="导入结果">
        <template slot-scope="scope">
          {{ `导入总数据${scope.row.total}条，覆盖${scope.row.cover}条，导入成功${scope.row.total - scope.row.error}条，导入失败${scope.row.error}条。` }}
        </template>
      </el-table-column>
      <el-table-column
        prop="option"
        ...
        <template slot-scope="scope">
          <template v-if="scope.row.total > 0">
            <el-button
              ...
              type="text"
              @click="downloadError(scope.row.error_data_file_path)">下载错误数据</el-button>
...
<script>
import { systemHistoryActivityListAPI } from '@/api/common'
import {
...
export default {
  ...
  methods: {
     ...
     */
    downloadError(path) {
      crmDownImportErrorAPI({ path, name: '导入错误数据' })
        .then(res => {
